...
academyRouter.get('/', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Seul le contenu publié est visible côté client — un brouillon créé
    // par l'automatisation d'upload admin (voir academy admin routes)
    // n'apparaît jamais tant que l'admin n'a pas cliqué "Publier".
    const contents = await AcademyContent.find({ status: 'publié' })
      .select('-sourceUrl')
      .sort({ createdAt: -1 });
