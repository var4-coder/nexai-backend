import { COMMISSION_NEXAI_TAUX } from '@/services/chariow.service';
import { MSG_COMMISSION_NEXAI } from '@/constants/textes-client';
...
usersRouter.patch('/me/payments', requireAuth, async (req, res, next) => {
  try {
    ...
    // Méthode de retrait / lien de paiement : Créateur+ et au-dessus (et admin).
    if (
      user.role !== 'admin' &&
      !['createur', 'agence', 'pro_max'].includes(String(user.plan))
    ) {
      throw new AppError(
    ...
    if (body.defaultPaymentMode === 'lien_personnel') {
      ...
      if (!link) {
        throw new AppError(
          'Indiquez le lien de votre page de paiement.',
          400
        );
      }
    ...
    res.json({
      ...
      // Transparence sur la commission : le client doit savoir ce qui est
      // prélevé AVANT de vendre, pas après sa première vente.
      commissionNexaiPourcent: Math.round(COMMISSION_NEXAI_TAUX * 100),
      partClientPourcent: Math.round((1 - COMMISSION_NEXAI_TAUX) * 100),
      explicationCommission: MSG_COMMISSION_NEXAI,
    });
  } catch (err) {
    next(err);
  }
