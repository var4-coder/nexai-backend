...
logosRouter.post('/generate', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Règle confirmée : génération de logo réservée aux abonnés payants —
    // essai gratuit ET Starter (académie uniquement) sont bloqués. Voir
    // assertLogoGenerationPlanAllowed dans credits.service.ts (même règle,
    // appliquée ici directement pour renvoyer le même message uniforme).
    if (user.plan === 'trial' || user.plan === 'starter') {
      throw new AppError('La génération de logo par IA est réservée aux abonnés payants (Créateur+, Agence ou Pro Max).', 403);
    }
    ...
    if (quota.canUseIncluded) {
      ...
      creditsBalanceAfter = await debitCredits(user._id, CREDIT_COSTS.LOGO, 'logo', {
