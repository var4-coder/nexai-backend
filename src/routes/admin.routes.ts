...
import { TexteCommercial } from '@/models/TexteCommercial';
import {
  proposerMiseAJourTextes,
  validerProposition,
  rejeterProposition,
  initialiserTextes,
} from '@/services/textes-commerciaux.service';
 ...
 */
/**
 * GET /galerie — vidéos candidates à la galerie d'exemples.
 *
 * Liste les vidéos terminées de l'administrateur : il les génère
 * normalement depuis son compte, puis choisit celles à exposer. La galerie
 * montre ainsi la vraie qualité NexAI, pas des vidéos retouchées ailleurs.
 */
// ══════════════════════════════════════════════════════════════════
// HÉBERGEMENT — lecture seule
//
// Le système applique seul le quota, les avertissements, la suspension et
// le rétablissement. L'administrateur consulte, il n'intervient pas.
// ══════════════════════════════════════════════════════════════════

adminRouter.get('/hebergement', requireRole('admin'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const sites = await Site.find({
      status: { $in: ['launched', 'offline'] },
      'hebergement.etape': { $ne: 'aucune' },
    })
      .select('name userId status hebergement subdomainSlug domainName')
      .sort({ 'hebergement.depassementDepuis': 1 })
      .limit(200)
      .lean();
    ...
    res.json({
      sites: sites.map((s) => ({
        id: String(s._id),
        nom: s.name,
        adresse: s.domainName || (s.subdomainSlug ? `${s.subdomainSlug}` : null),
        statut: s.status,
        etape: s.hebergement?.etape ?? 'aucune',
        visiteursMois: s.hebergement?.visiteursMois ?? 0,
        depassementDepuis: s.hebergement?.depassementDepuis ?? null,
        hebergementPayeJusquAu: s.hebergement?.payeJusquAu ?? null,
      })),
    });
  } catch (err) {
    next(err);
...
// ══════════════════════════════════════════════════════════════════
// DOMAINES — suivi des provisionnements et échéances
// ══════════════════════════════════════════════════════════════════

adminRouter.get('/domaines', requireRole('admin'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const domaines = await Domain.find({})
      .select('domainName status userId siteId expiresAt monthlyRenewalCredits provisionedCredits failedChargeCount')
      .sort({ expiresAt: 1 })
      .limit(300)
      .lean();
    ...
    res.json({
      domaines: domaines.map((d) => ({
        ...
        nom: d.domainName,
        statut: d.status,
        echeance: d.expiresAt,
        creditsParMois: d.monthlyRenewalCredits ?? 0,
        creditsProvisionnes: d.provisionedCredits ?? 0,
...
// ══════════════════════════════════════════════════════════════════
// TARIFS — textes commerciaux et leur mise à jour
// ══════════════════════════════════════════════════════════════════

adminRouter.get('/textes', requireRole('admin'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const textes = await TexteCommercial.find({}).sort({ emplacement: 1, cle: 1 }).lean();
    res.json({
      textes: textes.map((t) => ({
        cle: t.cle,
        ...
        role: t.role ?? null,
        contenu: t.contenu,
        proposition: t.propositionContenu ?? null,
        motif: t.propositionMotif ?? null,
        proposeLe: t.propositionLe ?? null,
        majLe: t.majLe,
        majPar: t.majPar ?? null,
      })),
      enAttente: textes.filter((t) => !!t.propositionContenu).length,
    });
  } catch (err) {
    next(err);
  }
...
/**
 * POST /textes/analyser — fait relire tous les textes par Sonnet après un
 * changement de tarif. Rien n'est publié : les propositions attendent la
 * validation de l'administrateur.
 */
/**
 * POST /textes/initialiser — crée les textes commerciaux de départ.
 *
 * Sans cela, l'onglet Tarifs reste vide : rien ne peuple cette collection au
 * démarrage, et l'administrateur n'a donc aucun texte à modifier. Les textes
 * déjà présents ne sont jamais écrasés — la commande est rejouable.
 */
adminRouter.post('/textes/initialiser', requireRole('admin'), async (_req: Request, res: Response, next: NextFunction) => {
  try {
    const r = await initialiserTextes();
    res.json({
      message:
        r.crees > 0
          ? `${r.crees} texte(s) ajouté(s). ${r.existants} étaient déjà présents et n'ont pas été touchés.`
          : 'Tous les textes sont déjà présents.',
      ...r,
    });
  } catch (err) {
    next(err);
...
adminRouter.post('/textes/analyser', requireRole('admin'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { valeurs } = z.object({ valeurs: z.string().min(10).max(4000) }).parse(req.body ?? {});
    const resultat = await proposerMiseAJourTextes(valeurs);
    res.json({
      message:
        resultat.propositions > 0
          ? `${resultat.propositions} texte(s) à mettre à jour. Relisez chaque proposition avant de publier.`
          : 'Tous les textes sont à jour, aucune correction nécessaire.',
      ...resultat,
    });
  } catch (err) {
    next(err);
...
/** POST /textes/:cle/valider — publie la proposition, éventuellement corrigée. */
adminRouter.post('/textes/:cle/valider', requireRole('admin'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { contenu } = z.object({ contenu: z.string().trim().min(1).max(5000).optional() }).parse(req.body ?? {});
    const texte = await validerProposition(req.params.cle, contenu);
    res.json({ message: 'Texte publié.', contenu: texte.contenu });
  } catch (err) {
    next(err);
  }
});

/** POST /textes/:cle/rejeter — écarte la proposition, rien n'est publié. */
adminRouter.post('/textes/:cle/rejeter', requireRole('admin'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    await rejeterProposition(req.params.cle);
    res.json({ message: 'Proposition écartée.' });
  } catch (err) {
    next(err);
  }
});
...
/**
 * POST /galerie/import — téléverser une vidéo d'exemple depuis ses fichiers.
 *
 * Même principe que l'Académie : l'administrateur choisit un fichier, lui
 * donne un titre, et il rejoint la vitrine. Utile pour montrer des exemples
 * qui n'ont pas été produits par le pipeline — une vidéo de démonstration,
 * une production retravaillée, un montage de présentation.
 *
 * La vidéo est stockée en PUBLIC : la galerie doit être visible sans compte,
 * c'est tout son intérêt commercial.
 */
adminRouter.post(
  '/galerie/import',
  requireRole('admin'),
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const fichier = await uploadVitrineVideo(req.file.buffer, req.file.originalname);

      // Enregistrée comme une vidéo terminée, mais marquée « importée » :
      // elle ne consomme aucun crédit et n'entre dans aucune statistique de
      // production.
      const video = await VideoAd.create({
        ...
        status: 'completed',
        creditsCharged: 0,
        vitrineImportee: true,
        finalVideoUrl: fichier.url,
        thumbnailUrl: fichier.miniatureUrl,
        brief: { brandName: body.titre },
        galerie: {
          publie: body.publie,
          titre: body.titre,
      ...
      await logEvent({
        ...
        niveau: 'info',
        message: `Vidéo de vitrine importée : « ${body.titre} »`,
      });

      res.status(201).json({
        message: body.publie
          ? 'Vidéo importée et publiée dans la galerie.'
          : 'Vidéo importée. Publiez-la pour la rendre visible.',
        id: String(video._id),
      });
    } catch (err) {
      next(err);
...
adminRouter.get('/galerie', requireRole('admin'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const videos = await VideoAd.find({ userId: req.auth!.userId, status: 'completed' })
      .select('galerie finalVideoUrl thumbnailUrl format mode createdAt')
      .sort({ createdAt: -1 })
      .limit(100)
      .lean();
    res.json({
      videos: videos.map((v) => ({
        ...
        format: v.format,
        mode: v.mode,
        videoUrl: v.finalVideoUrl ?? null,
        miniatureUrl: v.thumbnailUrl ?? null,
        creeLe: v.createdAt,
        galerie: v.galerie ?? { publie: false, titre: '', description: '', ordre: 0, categorie: 'exemple' },
      })),
    });
  } catch (err) {
    next(err);
...
/**
 * PATCH /galerie/:id — publier ou retirer une vidéo de la galerie, et régler
 * son titre, sa description, son ordre et sa catégorie.
 */
adminRouter.patch('/galerie/:id', requireRole('admin'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const body = z
      .object({
        publie: z.boolean(),
    ...
    const video = await VideoAd.findOne({ _id: req.params.id, userId: req.auth!.userId });
    if (!video) throw new AppError('Vidéo introuvable.', 404);
    if (body.publie && !video.finalVideoUrl) {
      throw new AppError('Seule une vidéo terminée peut être publiée dans la galerie.', 400);
    }
    if (body.publie && !body.titre) {
      throw new AppError('Donnez un titre à la vidéo avant de la publier.', 400);
    }
    ...
    await video.save();
    res.json({ message: body.publie ? 'Vidéo publiée dans la galerie.' : 'Vidéo retirée de la galerie.', galerie: video.galerie });
  } catch (err) {
    next(err);
  }
});
...
adminRouter.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      // Onglet unique « Sites à traiter ». Deux situations différentes, mais
      // la même action pour l'administrateur : regarder, puis reprendre.
      //  · 'failed'          — la génération a planté, rien n'a été produit
      //  · 'pending_support' — un site existe, mais sa qualité est insuffisante
      ...
      const sites = await Site.find({ status: { $in: statuts } })
        .populate('userId', 'email')
        .sort({ updatedAt: -1 })
        .limit(100);

      const [echecs, qualite] = await Promise.all([
        Site.countDocuments({ status: 'failed' }),
        Site.countDocuments({ status: 'pending_support' }),
      ]);

      res.json({ sites, compteurs: { echecTechnique: echecs, qualiteInsuffisante: qualite } });
    } catch (err) {
      next(err);
    }
  }
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { limit } = z
        ...
        .parse(req.body ?? {});
      if (!env.ADMIN_EMAIL) {
        throw new AppError("Aucune adresse email administrateur n'est configurée.", 400);
      }
      ...
      const { sendAdminLogsDigestEmail } = await import('@/services/brevo.service');
      await sendAdminLogsDigestEmail({
        to: env.ADMIN_EMAIL,
        logs: logs.map((l) => ({
      ...
      res.json({ ok: true, envoyes: logs.length, destination: env.ADMIN_EMAIL });
    } catch (err) {
      next(err);
    }
  }
...
adminRouter.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Deux catégories distinctes dans l'administration :
      //  · 'serieuse' — seul l'administrateur peut agir (clé, crédits,
      //    panne externe, bug du code). Un email est déjà parti.
      //  · 'fable'    — les agents s'en chargent seuls.
      if (req.query.categorie === 'serieuse' || req.query.categorie === 'fable') {
      ...
      const incidents = await PlatformAlert.find(filtre)
        ...
        .limit(100)
        .lean();
      // Compteurs par catégorie, pour les pastilles de l'interface.
      const [serieusesOuvertes, fableOuvertes] = await Promise.all([
        PlatformAlert.countDocuments({ categorie: 'serieuse', statut: { $ne: 'resolu' } }),
        PlatformAlert.countDocuments({ categorie: 'fable', statut: { $ne: 'resolu' } }),
      ]);
      res.json({
        compteurs: { serieuses: serieusesOuvertes, fable: fableOuvertes },
