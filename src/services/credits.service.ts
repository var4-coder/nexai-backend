...
export async function debitCredits(
  ...
  const user = await User.findById(userId).select('plan role creditsBalance trialEndsAt planExpiresAt');
  if (!user) throw new AppError('Utilisateur introuvable', 404);
  ...
  // ── Abonnement expiré ──
  //
  // Point de passage UNIQUE de toutes les actions payantes : contrôler ici
  // garantit qu'aucune n'est oubliée. Un abonné dont la période payée est
  // terminée garde l'étiquette de son plan, mais ne produit plus rien — même
  // s'il lui reste des crédits.
  //
  // Seule exception : l'hébergement seul, qui s'adresse précisément aux
  // comptes sans abonnement actif pour garder leur site en ligne.
  if (!aUnAbonnementActif(user) && user.plan !== 'trial' && type !== 'hebergement_seul') {
    throw new AppError(
      'Votre abonnement n’est plus actif. Reprenez votre abonnement pour continuer à créer et modifier vos sites.',
      403
    );
  }
...
/**
 * Coût fournisseur réel d'une vidéo, en dollars — pour le suivi de
 * rentabilité dans l'administration.
 *
 * Indexé par MODE et DURÉE uniquement : la qualité ne se choisit plus, le
 * moteur découle de la durée (Alexya sur les formats courts, Kling sur les
 * longs, voir getVideoEngineForFormat). La voix off ajoute le coût de la
...
export function getVideoAdCreditCost(
  ...
  // Conservé dans la signature pour la compatibilité des appels existants,
...
/**
 * Moteur vidéo, déterminé par la durée.
 *
 * Alexya ne génère que des plans de 10 s en mode « best » et accumule des
 * incohérences visuelles sur les formats longs (chaque plan est généré sans
 * mémoire du précédent). Kling gère la cohérence entre plans : il sert donc
 * les formats longs, où le client paie pour la qualité.
 */
export function getVideoEngineForFormat(format: VideoAdFormat): 'alexya' | 'kling' {
  return format === '20s' || format === '30s' ? 'alexya' : 'kling';
}
...
export function assertVideoAdPlanAllowed(plan: UserPlan) {
  if (plan === 'trial') {
    throw new AppError(
      "L'outil vidéo est visible pendant l'essai, mais la génération est réservée aux abonnements. Découvrez nos réalisations dans la galerie d'exemples, et abonnez-vous dès Créateur+ pour créer vos propres publicités.",
      403
    );
  }
