...
/**
 * Nombre de clients gérables dans l'Espace Agence, par plan.
 * `null` = illimité. Les plans sans accès Agence n'atteignent jamais ce
 * contrôle (les routes /agency les rejettent en amont).
 */
export const AGENCY_CLIENT_LIMIT: Partial<Record<UserPlan, number | null>> = {
  agence: 10,
  pro_max: null,
};
...
export async function createClient(
  ...
  // Plafond de clients : différenciateur Agence -> Pro Max. Une agence qui
  // grandit doit pouvoir monter en gamme, c'est le levier d'upgrade le plus
  // naturel sur ce segment.
  const agency = await User.findById(agencyUserId).select('plan role');
  if (!agency) throw new AppError('Utilisateur introuvable', 404);

  const cap = AGENCY_CLIENT_LIMIT[agency.plan];
  if (agency.role !== 'admin' && cap !== null && cap !== undefined) {
    const current = await Client.countDocuments({ agencyUserId });
    if (current >= cap) {
      throw new AppError(
        `Votre abonnement Agence gère jusqu'à ${cap} clients. Passez à Pro Max pour un nombre de clients illimité.`,
        403
      );
    }
  }
