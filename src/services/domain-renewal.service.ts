...
export async function runDomainRenewalProvisioning(): Promise<{
  ...
  for (const domain of domains) {
    // ── Expiration réelle ──
    //
    // L'échéance est passée sans renouvellement : le domaine n'appartient
    // plus au client. Le site ne doit surtout pas devenir inaccessible — il
    // retrouve son adresse NexAI, qui redevient l'adresse principale.
    if (domain.expiresAt.getTime() < now.getTime()) {
      await replierSurSousDomaine(domain);
...
/**
 * Replie un site sur son sous-domaine NexAI après expiration de son domaine
 * personnalisé. Ne lève jamais d'erreur : un échec est journalisé, et le
 * prochain passage retentera.
 */
async function replierSurSousDomaine(domain: IDomain): Promise<void> {
  if (!domain.siteId) return;
  try {
    const site = await Site.findById(domain.siteId).select('netlifySiteId subdomainSlug domainType domainName');
    if (!site?.netlifySiteId || !site.subdomainSlug) {
      console.warn(`[domaines] Repli impossible pour ${domain.domainName} : sous-domaine inconnu`);
      return;
    }
    await revertToSubdomain(site.netlifySiteId, site.subdomainSlug);
    site.domainType = 'sous_domaine';
    site.domainName = undefined;
    await site.save();
    console.log(`[domaines] ${domain.domainName} expiré — site replié sur ${site.subdomainSlug}`);
  } catch (err) {
    console.error(`[domaines] Repli sur sous-domaine échoué pour ${domain.domainName}`, err);
  }
}
