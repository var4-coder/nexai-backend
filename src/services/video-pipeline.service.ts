    ...
    for (let i = 0; i < videoAd.scenes.length; i++) {
      ...
      for (let attempt = 0; attempt < 2 && !succeeded; attempt++) {
        try {
          ...
          // Qualité Standard = Alexya "best" · Premium = Alexya "cinematic"
          // SANS son (la voix vient toujours de notre narration TTS ajoutée
          // après montage — jamais le son Alexya, quelle que soit la
          // qualité, voir Architecture v6 section 8).
          const clip = await generateAlexyaVideoClip({
            prompt: scene.prompt,
            mode: videoAd.quality === 'premium' ? 'cinematic' : 'best',
