...
export async function enqueueVideoAdRelaunch(
  ...
  // Plafond des relances après échec total, puis REMBOURSEMENT.
  //
  // Le client n'a rien reçu : ni vidéo complète, ni vidéo partielle. Insister
...
/**
 * Relances gratuites autorisées après un échec TOTAL.
 *
 * Deux, puis remboursement. Une troisième tentative a très peu de chances
 * d'aboutir là où deux ont échoué, et laisserait le client attendre encore
 * sans rien avoir reçu pour son argent.
 */
const MAX_RELANCES_ECHEC_TOTAL = 2;

/**
 * Pause avant la seconde tentative d'un plan ou d'un segment.
 *
 * Assez longue pour qu'une saturation retombe, assez courte pour ne pas
 * peser sur la durée annoncée au client.
 */
const RETRY_PLAN_DELAY_MS = 20_000;
...
async function rembourserVideoNonLivree(video: {
  ...
  try {
    await creditCredits(String(video.userId), montant, 'ajustement_admin', {
      note: `Remboursement — vidéo non produite après ${MAX_RELANCES_ECHEC_TOTAL} tentatives`,
    });
    console.warn(
 ...
 *
 * DEUX au maximum, quel que soit le format. Chaque relance régénère de vrais
 * plans, à nos frais : sur un format court, une troisième tentative efface
 * la marge de la commande.
 *
 * Deux suffisent : un incident passager se règle presque toujours au
 * deuxième essai. Au-delà, le problème est ailleurs, et insister ne fait que
 ...
 */
/**
 * Images utilisables pour remplacer un plan qui n'a pas pu être animé.
 *
 * Par ordre de valeur commerciale : les photos produit du client d'abord —
 * ce sont elles qui vendent —, puis son logo. Renvoyer plusieurs images
 * permet de VARIER : deux segments remplacés ne montrent pas la même photo,
 * ce qui évite l'impression de vidéo figée.
 */
function imagesDeSecours(videoAd: {
  brief?: unknown;
  logoUrl?: string;
  thumbnailUrl?: string;
}): string[] {
  const brief = (videoAd.brief ?? {}) as { clientProductImageUrls?: string[] };
  const images = [
    ...(Array.isArray(brief.clientProductImageUrls) ? brief.clientProductImageUrls : []),
    videoAd.logoUrl,
    videoAd.thumbnailUrl,
  ].filter((u): u is string => typeof u === 'string' && u.length > 0);

  // Doublons retirés : la même image deux fois n'apporte aucune variation.
  return Array.from(new Set(images));
}

/**
 * Colle une piste audio sur une vidéo muette, sans réencoder l'image.
 *
 * Sert aux plans de remplacement : la narration du passage est conservée,
 * seule l'image change. Le message du client passe donc en entier, même
 * quand le présentateur n'a pas pu être animé.
 */
async function runFfmpegCollerAudio(params: {
  videoPath: string;
  audioPath: string;
  outputPath: string;
}): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    const proc = spawn('ffmpeg', [
      ...
      '-c:a', 'aac',
      '-b:a', '192k',
      // La sortie s'arrête au flux le plus court : l'image et le son
      // gardent ainsi exactement la même durée.
      '-shortest',
      outputPath,
    ]);
    let stderr = '';
    ...
    proc.on('close', (code) =>
      code === 0
        ? resolve()
        : reject(new AppError(`ffmpeg collage audio échoué: ${stderr.slice(-300)}`, 500))
    );
...
async function runVoixOffPipeline(
  ...
  const nbScenes = CLIPS_PER_FORMAT[videoAd.format as VideoAdFormat];
    ...
    const processScene = async (i: number): Promise<void> => {
      ...
      for (let attempt = 0; attempt < 2 && !succeeded; attempt++) {
        try {
          ...
          if (attempt === 1) {
            scene.retried = true;
            // Pause avant la seconde tentative.
            //
            // Les deux essais s'enchaînaient immédiatement : une saturation
            // passagère du fournisseur les faisait échouer tous les deux, et
            // le plan était déclaré manquant alors qu'il aurait abouti
            // quelques secondes plus tard.
            await sleep(RETRY_PLAN_DELAY_MS);
          }
          await queueSave();
    ...
    if (scenesEchouees > 0) {
      ...
      // Message adapté à ce que le client va réellement voir.
      //
      // La séquence d'images a-t-elle comblé tout le manque ? Si oui, la
      // vidéo fait bien la durée commandée et le client a surtout besoin de
      // comprendre pourquoi une partie est en images fixes. Sinon, sa vidéo
      // est plus courte que prévu, ce qui demande une explication franche.
      const manqueComble =
        secondesAnimees + dureeSequenceCloture(format, secondesAnimees) >=
        dureeCibleSecondes(format);
      videoAd.clientMessage = manqueComble
        ? MSG_VIDEO_SEQUENCES_REMPLACEES
        : MSG_VIDEO_PLUS_COURTE;
    ...
    if (dureeCloture > 0) {
      try {
        // Plusieurs images plutôt qu'une seule.
        //
        // Une photo unique tenue dix secondes donne l'impression d'une vidéo
        // figée. En faisant défiler les images dont on dispose — photos
        // produit du client d'abord, puis capture du site, puis logo — la
        // fin reste vivante et montre davantage de l'offre.
        const imagesCloture = [
          ...referencePool.productImages.map((im) => im.url),
          ...referencePool.mockupImages.map((im) => im.url),
          videoAd.logoUrl,
        ].filter((u): u is string => typeof u === 'string' && u.length > 0);
        const imagesUniques = Array.from(new Set(imagesCloture));

        if (imagesUniques.length > 0) {
          // Au moins 4 secondes par image : en dessous, le défilement
          // devient nerveux et on ne voit plus le produit.
          const nbImages = Math.max(1, Math.min(imagesUniques.length, Math.floor(dureeCloture / 4)));
          const dureeParImage = dureeCloture / nbImages;

          const briefCloture = videoAd.brief as { brandName?: string; ctaText?: string };
          const langueCloture = await langueDuProprietaire(videoAd.userId);
          const clipsCloture: string[] = [];

          for (let k = 0; k < nbImages; k++) {
            const srcPath = await downloadToTmp(imagesUniques[k], `${videoAdId}_closing_src${k}`);
            tmpFiles.push(srcPath);
            const partPath = path.join(os.tmpdir(), `${videoAdId}_closing_${k}.mp4`);
            tmpFiles.push(partPath);
            await runFfmpegBuildClosingCardClip({
              ...
              height: target.height,
              fps: target.fps,
              durationSeconds: dureeParImage,
              // Le nom de marque et l'appel à l'action n'apparaissent que
              // sur la DERNIÈRE image : répétés sur chacune, ils
              // clignoteraient.
              brandName: k === nbImages - 1 ? briefCloture?.brandName : undefined,
              ctaText: k === nbImages - 1 ? briefCloture?.ctaText : undefined,
              langue: langueCloture,
            });
            clipsCloture.push(partPath);
          }

          const closingClipPath = path.join(os.tmpdir(), `${videoAdId}_closing.mp4`);
          tmpFiles.push(closingClipPath);
          if (clipsCloture.length === 1) {
            await fs.copyFile(clipsCloture[0], closingClipPath);
          } else {
            await runFfmpegConcat(clipsCloture, closingClipPath);
          }
...
async function runAvatarPipeline(
  ...
  const clipLocalPaths: string[] = [];
  // Segments remplacés par des images faute d'avoir pu être animés.
  let segmentsRemplaces = 0;
  const resolutionCible = getTargetResolution(videoAd.aspectRatio);
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    for (let attempt = 0; attempt < 2 && !succeeded; attempt++) {
      try {
        // Même pause que sur les plans : laisser retomber une saturation
        // plutôt que de relancer dans la seconde.
        if (attempt === 1) await sleep(RETRY_PLAN_DELAY_MS);
    ...
    if (!succeeded) {
      // ── Le segment ne jette PLUS toute la vidéo ──
      //
      // Avant, un segment raté faisait échouer l'ensemble : les segments déjà
      // produits — et déjà payés au fournisseur — étaient jetés, et le client
      // ne recevait rien. Sur un mini-film de deux minutes, cela revenait à
      // payer une minute de vidéo que personne ne verrait jamais.
      //
      // À la place, on construit un plan de remplacement à partir des images
      // de la marque, et on y garde la NARRATION du segment : la voix
      // continue, le message passe en entier, seul le présentateur disparaît
      // le temps de ce passage.
      console.warn(
        `[video-pipeline] Segment avatar ${i} échoué après retry (${lastError}) — remplacement par les images de la marque`
      );
      segmentsRemplaces += 1;

      // On varie l'image quand le client en a fourni plusieurs : deux
      // segments remplacés ne montrent pas la même photo.
      const imagesDispo = imagesDeSecours(videoAd);
      const imageSecours = imagesDispo[(segmentsRemplaces - 1) % Math.max(1, imagesDispo.length)];

      if (!imageSecours) {
        // Aucune image disponible : on ne peut rien construire pour ce
        // segment. Les autres segments restent valides et seront livrés.
        console.warn(`[video-pipeline] Aucune image de secours pour le segment ${i}`);
        continue;
      }

      try {
        const srcPath = await downloadToTmp(imageSecours, `${videoAdId}_seg${i}_src`);
        tmpFiles.push(srcPath);
        const muetPath = path.join(os.tmpdir(), `${videoAdId}_seg${i}_muet.mp4`);
        tmpFiles.push(muetPath);
        const avecAudioPath = path.join(os.tmpdir(), `${videoAdId}_seg${i}_images.mp4`);
        tmpFiles.push(avecAudioPath);

        const briefSecours = videoAd.brief as { brandName?: string; ctaText?: string };
        await runFfmpegBuildClosingCardClip({
          imagePath: srcPath,
          outputPath: muetPath,
          width: resolutionCible.width,
          height: resolutionCible.height,
          fps: resolutionCible.fps,
          durationSeconds: segDuration,
          brandName: briefSecours?.brandName,
          ctaText: briefSecours?.ctaText,
          langue: videoAd.langue,
        });

        await runFfmpegCollerAudio({
          videoPath: muetPath,
          audioPath: segmentAudioPath,
          outputPath: avecAudioPath,
        });

        clipLocalPaths.push(avecAudioPath);
      } catch (err) {
        // Le plan de remplacement a échoué : on continue sans lui plutôt que
        // de perdre les segments réussis.
        console.error(`[video-pipeline] Plan de remplacement du segment ${i} échoué`, err);
      }
    }
  }

  // Aucun segment exploitable : rien à livrer. Les crédits restent acquis
  // à la commande et une relance gratuite s'ouvre au client.
  if (clipLocalPaths.length === 0) {
    throw new AppError(
      "Aucun segment n'a pu être produit pour cette vidéo.",
      502
    );
  }

  // Segments remplacés par des images : le client doit le savoir, et une
  // relance gratuite doit lui être ouverte pour les récupérer.
  if (segmentsRemplaces > 0) {
    videoAd.partialDelivery = {
      scenesGenerated: clipLocalPaths.length - segmentsRemplaces,
      scenesFailed: segmentsRemplaces,
    };
    videoAd.clientMessage = MSG_VIDEO_SEQUENCES_REMPLACEES;
    await videoAd.save();
  }
