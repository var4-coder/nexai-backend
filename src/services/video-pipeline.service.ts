...
export async function enqueueVideoAdRelaunch(
  ...
  const relanceGratuite = echecTotal && (original.creditsCharged ?? 0) > 0;
  ...
  if (relanceGratuite) {
    ...
    if (depuisEchec < FREE_RELAUNCH_COOLDOWN_MS) {
      const minutes = Math.ceil((FREE_RELAUNCH_COOLDOWN_MS - depuisEchec) / 60000);
      throw new AppError(
        `Le service se rétablit. Vous pourrez relancer gratuitement dans ${minutes} minute${minutes > 1 ? 's' : ''}.`,
        429
      );
    }
    cost = 0;
  } else if (relancePartielle) {
    if (relancesDejaFaites >= MAX_PARTIAL_RELAUNCHES) {
      throw new AppError(
...
/**
 * Exception à la règle des 50 % : le format 30 s.
 *
 * Un seul plan réussi (10 s, soit 33 %) complété par la séquence de clôture
 * donne 25 s sur 30 — un format court mais pleinement exploitable, jugé
 * préférable à un non-livré. La règle des 50 % reste la règle générale et
 * s'applique intégralement aux formats 60 s et 120 s, où un manque équivalent
 * produirait une vidéo trop amputée pour être présentable.
 */
const FORMATS_AVEC_EXCEPTION_SEUIL: ReadonlySet<VideoAdFormat> = new Set(['30s']);
 ...
 *
 * 3 est un bon compromis : la 3ème relance ne survient que si les deux
 * premières ont échoué, soit environ 1 vidéo sur 1000 — son coût agrégé est
 * négligeable, et elle rattrape le client le plus malchanceux.
 */
const MAX_PARTIAL_RELAUNCHES = 3;
 ...
 */
function getSceneConcurrency(quality: VideoAdQuality): number {
  const raw =
    quality === 'premium'
      ? process.env.WORKER_VIDEO_SCENE_CONCURRENCY_PREMIUM
      : process.env.WORKER_VIDEO_SCENE_CONCURRENCY_STANDARD;
  const parsed = raw ? Number.parseInt(raw, 10) : NaN;
  if (Number.isFinite(parsed) && parsed >= 1) return Math.min(parsed, 6);
  return quality === 'premium' ? 1 : 3;
}
...
async function runVoixOffPipeline(
  ...
  const nbScenes = CLIPS_PER_FORMAT[videoAd.format as VideoAdFormat];
    ...
    const processScene = async (i: number): Promise<void> => {
      ...
      for (let attempt = 0; attempt < 2 && !succeeded; attempt++) {
        try {
          ...
          // milieu de la pub. Le moteur découle donc uniquement de
          // videoAd.quality, décidé à l'achat et jamais modifié en cours de
          // génération.
          //
    ...
    await runWithConcurrency(
      ...
      getSceneConcurrency(videoAd.quality),
      processScene
    );
...
async function runAvatarPipeline(
  ...
  // Qualité FalAI = choix réel du client (Standard/Premium), jamais déduite
  // du mode — mini_film a lui aussi ses deux niveaux de qualité (140cr
  // Standard / 280cr Premium, voir credits.service.ts).
  const quality: AvatarQuality = videoAd.quality === 'premium' ? 'pro' : 'standard';
  ...
  const estTestEssai = (await User.findById(videoAd.userId).select('plan'))?.plan === 'trial';
