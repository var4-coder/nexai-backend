...
import { runScan1, runScan2, formatScanIssues } from '@/services/scan.service';
...
export async function enqueueSiteGeneration(
  ...
  // Le solde renvoyé par debitCredits est remonté jusqu'à la réponse HTTP
  // (voir sitesRouter POST /:id/generate) pour que le frontend puisse
  // mettre à jour l'affichage immédiatement, sans dépendre d'un second
  // appel de rafraîchissement qui pouvait être oublié côté client.
...
export async function processGeneration(
  ...
  for (let i = 1; i <= previewCount; i++) {
    ...
    if (!scan1.ok) {
      ...
      // Réparation immédiate par Grok Build AVANT de solliciter le Juge Code.
...
export async function enqueueLaunch(
  ...
  if (opts.domainType === 'sous_domaine') {
    ...
    // sous-domaine (Netlify refuserait le 2e custom_domain, et l'échec
    // était auparavant avalé en silence côté worker — le client se
    // retrouvait avec un site sans sous-domaine, sans le savoir). On
    // cherche donc ici le premier slug libre, en ajoutant -2, -3, etc.
