 ...
 *
 * Le worker traite ses jobs l'un après l'autre à l'intérieur d'un même flux,
 * et le compteur est posé au début de chaque génération puis retiré à la
 * fin : deux générations ne peuvent pas mélanger leurs comptes.
 */
let compteurActif: CompteurDepense | null = null;

/**
 * Comptabilise le dernier appel IA effectué. Sans effet si aucune
 * génération n'est en cours.
 */
function compterDernierAppel(): void {
  if (!compteurActif || !dernierUsage) return;
  compteurActif.ajouter(dernierUsage.modele, dernierUsage.entree, dernierUsage.sortie);
  reinitialiserUsage();
}
...
export async function processGeneration(
  ...
  // Codeur effectivement utilisé pour chaque aperçu. Sert à la règle du
  // juge : un modèle ne juge jamais sa propre production.
  ...
  for (let i = 1; i <= previewCount; i++) {
    ...
    //
    //    Répartition :
    //      · Premium          → codeur_premium (Opus 5)
    //      · Normale, aperçu 2 → codeur_normale_apercu2 (Sonnet 5) : un moteur
    //        différent du premier, pour deux propositions réellement distinctes
    //      · Essai et aperçu 1 → codeur_normale (Grok 4.6, alternable)
  ...
  if (sousSeuil) {
    ...
    // dernière. On n'ouvre pas une nouvelle alerte que Fable reprendrait —
    // ce serait une boucle sans fin, chaque tour refabriquant le site à nos
    // frais. Le site est escaladé à l'administrateur.
    const escalade = refabricationFable && !estEssai;
    ...
    try {
      await AlerteQualite.create({
        ...
        // Escalade : le site est livré tout de suite (voir plus bas), l'alerte
        // est donc close dès sa création — elle sert d'historique pour
        // l'administrateur, pas de file d'attente.
        statut: estEssai || escalade ? 'traitee_auto' : 'ouverte',
        escaladeAdmin: escalade,
        niche: site.niche,
        plan,
        verdictJuges,
        score: bestScore || undefined,
        traitePar: estEssai || escalade ? 'systeme' : undefined,
        traiteA: estEssai || escalade ? new Date() : undefined,
      });
    } catch (err) {
    ...
    if (escalade) {
      // Avec le contrôle de clarté en amont, la demande du client est
      // claire : un échec à ce stade signale un vrai problème du système.
      signalerIncident({
        composant: 'generation-site',
        erreur:
          `Site toujours insuffisant après la refabrication de Fable (score ${bestScore || 0}), ` +
          `livré tel quel au client. Niche ${site.niche}. À examiner : demande du client, ` +
          `verdict des juges, versions produites — pour identifier une éventuelle cause système.`,
        contexte: `sites-a-traiter:${site.niche}`,
        gravite: 'critique',
      }).catch(() => {});
    }
    ...
    // Essai : livré quand même.
    // Échec après la refabrication de Fable : livré aussi — refaire encore
    // aboutirait au même résultat, et le client ne doit pas attendre pour
    // rien. Il peut ensuite l'améliorer lui-même, par l'IA, ou relancer.
    // Payant, premier échec : en attente de la décision de Fable.
    site.coutGenerationUsd = Number(depense.totalUsd.toFixed(4));
    site.coutParModele = depense.repartition();
    site.status = estEssai || escalade ? 'ready' : 'pending_support';
    console.warn(
