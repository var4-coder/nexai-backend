...
async function replaceOneCollection(db: mongoose.mongo.Db, name: string): Promise<SeedResult> {
  ...
  // laisserait des documents orphelins si les identifiants du contenu
  // diffèrent de ceux déjà en base — ici, la collection correspond
  // exactement au contenu inséré, rien de plus.
 ...
 * Vide chaque collection puis insère le contenu de la Librairie : garantit
 * qu'aucun document résiduel ne subsiste, même si les identifiants diffèrent
 * de ceux déjà en base (ex. nombre de niches différent). À utiliser
 * volontairement, quand on pousse une mise à jour du contenu.
 */
