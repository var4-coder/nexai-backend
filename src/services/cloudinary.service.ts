...
/**
 * Supprime un plan vidéo intermédiaire archivé pour une relance partielle.
 *
 * Ces plans sont des fichiers de travail (les "rushes"), jamais visibles ni
 * téléchargeables par le client : seule la vidéo finale montée lui est
 * livrée, et celle-ci n'est JAMAIS supprimée. On efface les rushes dès qu'une
 * relance ne peut plus en avoir besoin, pour ne pas saturer le stockage avec
 * des fichiers devenus inutiles.
 *
 * Les vidéos pub sont uploadées en type 'upload' (public), pas
 * 'authenticated' : la suppression doit utiliser le même type.
 */
export function deleteVideoAdClip(publicId: string): Promise<void> {
  ensureConfigured();
  return cloudinary.uploader
    .destroy(publicId, { resource_type: 'video', type: 'upload' })
    .then(() => undefined)
    .catch(() => undefined);
}
...
/**
 * Téléverse une vidéo de VITRINE, accessible publiquement.
 *
 * Contrairement aux vidéos de l'Académie, stockées en mode authentifié et
 * protégées par une URL signée, celles-ci doivent être visibles par
 * n'importe quel visiteur — y compris sans compte. C'est tout l'intérêt de
 * la galerie d'exemples : montrer la qualité avant l'inscription.
 *
 * Renvoie l'URL publique directement utilisable, et non un identifiant.
 */
export function uploadVitrineVideo(
  buffer: Buffer,
  filenameHint: string
): Promise<{ url: string; publicId: string; miniatureUrl: string }> {
  ensureConfigured();
  return new Promise((resolve, reject) => {
    const upload = cloudinary.uploader.upload_stream(
      {
      ...
      (error, result) => {
        ...
        resolve({
          ...
          // Miniature extraite de la vidéo par Cloudinary, à la volée.
          miniatureUrl: result.secure_url.replace(/\.[^.]+$/, '.jpg'),
        });
      }
    );
    upload.end(buffer);
