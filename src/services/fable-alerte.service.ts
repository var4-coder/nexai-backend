import { AlerteQualite, IAlerteQualite } from '@/models/AlerteQualite';
...
export async function livrerAlertesBloquees(): Promise<number> {
  ...
  for (const alerte of bloquees) {
    ...
    await logEvent({
      categorie: 'alerte_qualite',
      niveau: 'warn',
      message: `Livraison de secours — aucune décision en 1h (niche ${alerte.niche}). Site livré au client avec badge « Meilleure version ».`,
      siteId: site._id,
      userId: alerte.userId,
    });
    livrees++;
