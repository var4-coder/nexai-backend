...
async function callDialogueTurn(session: InstanceType<typeof ChatSession>) {
  ...
  for (let attempt = 0; attempt <= MAX_DIALOGUE_RETRIES; attempt++) {
    lastRaw = await callClaude(
      ...
      {
        // 500 → 800 : le mode 'business' peut légitimement produire un
        // message plus long (jusqu'à 3 idées + pitchs) ; à 500 tokens la
        // réponse JSON pouvait être coupée avant la accolade fermante,
        // ce qui invalidait le parsing à coup sûr.
        maxTokens: 800,
        temperature: 0.4,
      }
    );
