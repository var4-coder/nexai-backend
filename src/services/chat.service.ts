...
- DESCRIPTION D'ACTIVITÉ (exigence qualité, sans exagérer) :
  • Refuse une description trop vague (1-2 mots, "coaching", "services", "business", "vente en ligne" seul).
  • Exige au minimum : ce que le client fait ou vend + pour qui (une vraie phrase, ~6 mots ou plus).
  • Si trop vague : mode "input", message clair du type "C'est un peu trop vague pour un bon site — peux-tu préciser ce que tu proposes et à qui ?" — ne mets PAS readyForExtraction: true.
  • N'exige pas un roman : une phrase concrète suffit (ex. "Je propose du coaching LinkedIn aux freelances qui veulent plus de clients").
- Une fois niche, nom, description SUFFISAMMENT PRÉCISE et public cible couverts, demande OBLIGATOIREMENT la préférence de logo en mode "choices" avec ces options exactes : ${logoOptions}.
- Dès que niche, nom, description précise, public, style ET préférence logo sont couverts → "readyForExtraction": true.
...
export async function startChatSession(
  ...
  // Coach business — 6 crédits débités IMMÉDIATEMENT au démarrage, avant
...
export async function confirmChatSession(sessionId: string, userId: string, confirmed: boolean) {
  ...
  if (logoPreference === 'has_logo') {
    ...
    try {
      ...
      // signale explicitement au frontend via pendingLogoAction, au lieu
      // de l'ancien comportement qui livrait le site sans logo en
      // silence. Le site part sans logo ; le client peut en ajouter un
