...
export interface IChatMessage {
  ...
  options?: string[];
  /**
   * Suggestion de cross-vente vers un autre mode du hub (site/logo/edit/business),
   * proposée par l'IA en fin de flux. Le frontend l'affiche comme un CTA
   * ("On crée ton site autour de ce logo ?") qui, si cliqué, appelle
   * switchChatMode() pour continuer SANS perdre le contexte déjà collecté.
   */
  suggestMode?: ChatHubMode;
...
export interface IChatSession {
  ...
  /**
   * Étape après extraction du brief site, pilotée par le backend
   * (offre de textes → qualité Standard/Premium → récap).
   */
  postBriefStep?: 'copy_offer' | 'copy_review' | 'quality' | 'recap';
  createdAt: Date;
  updatedAt: Date;
}
...
const chatSessionSchema = new Schema<IChatSession>(
  {
    ...
    postBriefStep: {
      type: String,
      enum: ['copy_offer', 'copy_review', 'quality', 'recap'],
    },
    niche: {
      type: String,
      enum: [
