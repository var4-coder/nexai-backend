...
export interface ISite {
  ...
  /**
   * Sous-domaine NexAI du site, conservé de façon permanente.
   *
   * Indispensable au repli : si un domaine personnalisé expire, le site
   * retrouve cette adresse au lieu de devenir inaccessible.
   */
  subdomainSlug?: string;
  /**
   * Coût fournisseur RÉEL de la génération, en dollars, calculé à partir des
   * tokens effectivement consommés — pas d'une estimation. Alimente le suivi
   * de rentabilité dans l'administration.
   */
  coutGenerationUsd?: number;
  /** Répartition de ce coût par modèle, pour comprendre d'où il vient. */
  coutParModele?: Record<string, number>;
  /**
   * Suivi de l'hébergement gratuit, pour un site dont le propriétaire n'a
   * plus d'abonnement actif (voir hebergement.service.ts).
   */
  hebergement?: {
    /** Premier jour où le quota mensuel de visiteurs a été dépassé. */
    depassementDepuis?: Date;
    /** Étape d'avertissement atteinte, pour ne jamais notifier deux fois. */
    etape?: 'aucune' | 'avertissement' | 'mode_limite' | 'dernier_avertissement' | 'suspendu';
    /** Hébergement seul payé jusqu'à cette date. */
    payeJusquAu?: Date;
    /** Nombre de visiteurs relevé lors du dernier contrôle. */
  ...
  /** Crédits débités pour la génération en cours / échouée (pour relance / remboursement). */
  ...
  freeRelaunchAvailable?: boolean;
  /** Le client a déjà utilisé sa relance gratuite. */
  freeRelaunchUsed?: boolean;
  /** Relance gratuite disponible à partir de cette date (compte à rebours 30 min). */
...
const siteSchema = new Schema<ISite>(
  {
    ...
    coutGenerationUsd: { type: Number },
    coutParModele: { type: Schema.Types.Mixed },
    hebergement: {
      depassementDepuis: { type: Date },
      etape: {
        type: String,
        enum: ['aucune', 'avertissement', 'mode_limite', 'dernier_avertissement', 'suspendu'],
        default: 'aucune',
      },
      payeJusquAu: { type: Date },
      visiteursMois: { type: Number, default: 0 },
    },
