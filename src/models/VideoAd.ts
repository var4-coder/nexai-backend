...
export interface IVideoAd {
  ...
  /**
   * Message expliquant au client ce qui s'est passé sur cette vidéo :
   * séquences remplacées par des images, durée plus courte que prévu.
   *
   * Sans ce champ au schéma, Mongoose ignorait silencieusement la valeur et
   * le client ne voyait jamais l'explication.
   */
  clientMessage?: string;
  /** Crédits rendus : la vidéo n'a jamais pu être produite. */
  rembourse?: boolean;
  rembourseA?: Date;
  /**
   * Vidéo TÉLÉVERSÉE par l'administrateur pour la vitrine, plutôt que
   * générée par le pipeline. Elle n'appartient à aucune commande client :
   * elle ne consomme aucun crédit, n'entre dans aucune statistique de
   * production, et sert uniquement d'exemple.
   */
  vitrineImportee?: boolean;
  galerie?: {
    publie: boolean;
    titre: string;
    description?: string;
